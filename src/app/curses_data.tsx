...
export const CURSES = [
  ...
  // ━━━ 🔵 中階咒文 | 人際擋箭 | Free ━━━
  {
    id: "parent_teacher_note_forge",
    tab: "人際擋箭",
    isPro: false,
    tier: "adept",
    school: "healing" as SchoolType,
    subSchool: "insight" as SchoolType,
    outputFormat: "親師溝通訊息 + 聯絡簿範本 + 接話備案",
    icon: <NotebookPen className="w-8 h-8 text-sky-500" />,
    color: "sky",
    title: "親師外交：家校溝通術",
    desc: "孩子回家說被同學推、想幫他請假去看醫生、覺得老師處罰太重想了解、或單純想謝謝老師——要跟老師開口，訊息卻在手機上打了又刪：寫太兇怕被貼上『恐龍家長』、寫太軟又怕老師敷衍不理、更怕一講反而害孩子在班上被針對？親師之間本來就隔著一層，你不了解課堂全貌，老師也帶著幾十個孩子，一句話沒說好就容易變成對立。這咒語當你的親師溝通顧問，把你想反映的事，寫成一則既守護孩子、又尊重老師專業的訊息——用『我們一起幫孩子』的合作口吻取代興師問罪，把訴求講清楚卻不失禮，附上可手寫進聯絡簿的精簡版、老師不同反應時你怎麼接話，還幫你避開那些會讓老師瞬間豎起防衛的地雷句，讓你不當恐龍家長也能替孩子把話說到位。",
    tags: ["親師溝通", "聯絡簿", "家長必備"],
    fields: [
      { id: "matter", label: "要跟老師溝通什麼事", placeholder: "例：孩子說被同學推倒想反映 / 明天要帶去看醫生請假 / 覺得處罰太重想了解 / 想謝謝老師特別照顧" },
      { id: "child", label: "孩子的狀況", placeholder: "例：小二、比較內向不敢講 / 幼兒園中班、最近抗拒上學 / 小五、被誤會作弊" },
      { id: "goal", label: "你希望達成什麼", placeholder: "例：老師多留意但別讓孩子難堪 / 順利請到假 / 了解來龍去脈不想吵架 / 單純表達感謝" },
      { id: "channel_tone", label: "溝通管道與你的顧慮", placeholder: "例：寫聯絡簿 / 傳 LINE 給老師 / 怕被當恐龍家長 / 怕孩子被貼標籤 / 想客氣但把話講清楚" },
    ],
    tweak: {
      id: "strat",
      label: "溝通策略",
      options: [
        "溫和協力型：站在『我們一起幫孩子』的立場，把訴求包裝成親師合作，老師更願意主動配合",
        "據理力爭型：態度客氣但立場堅定，把該爭取的權益與該釐清的事講清楚，不失禮又不被打發",
        "感謝連結型：先真誠肯定老師的付出再帶入需求，用善意鋪路，讓後續每一次溝通都更順",
      ]
    },
    modules: [
      { type: 'role' as ModuleType, label: '角色設定', preview: '資深親師溝通輔導老師' },
      { type: 'decision' as ModuleType, label: '判斷邏輯', preview: '依事件輕重調整語氣' },
      { type: 'output' as ModuleType, label: '輸出格式', preview: '訊息＋聯絡簿＋接話' },
      { type: 'safety' as ModuleType, label: '安全邊界', preview: '對事不對人不貼標籤' },
      { type: 'behavior' as ModuleType, label: '行為規則', preview: '禁恐龍家長式指責' },
    ],
    theory: "建立在兩個框架上：一是 Marshall Rosenberg 的『非暴力溝通』(NVC)——衝突多半來自把『觀察』說成了『指控』，正確順序是先客觀描述發生的事、再說出自己的感受與需要、最後提出具體可行的請求，而非一開口就評判對方（『你們怎麼可以…』），這樣對方才不會急著防衛。二是『親師是教育合夥關係』與基本歸因謬誤（Fundamental Attribution Error）——家長容易把單一事件直接歸因為『老師不用心／針對我孩子』，忽略情境因素；一旦預設惡意，語氣就會帶刺、把老師推到對立面。本咒語把這兩個框架翻譯成家長可以直接照抄的訊息：既守住孩子的權益，又把老師當成『和你目標一致、都想把孩子帶好』的合作夥伴，讓溝通從對立變協力。",
    generate: (inputs: any) => `你是一位資深的親師溝通輔導老師，同時當過導師與家長，協助過上百對親師化解誤會，最擅長把家長一肚子的擔心與不滿，翻譯成一則老師讀了會想幫忙、而不是想防備的訊息。你的信條：「家長和老師從來不是對手，是同一隊——把話說對，老師就成了你孩子在學校最有力的隊友。」\n\n採用策略：[[${inputs.strat}]]\n\n【任務】\n我要跟孩子的老師溝通一件事，想把話說到位、達成目的，又不傷親師關係、不害到孩子。\n- 要溝通什麼事：[[${inputs.matter}]]\n- 孩子的狀況：[[${inputs.child}]]\n- 我希望達成什麼：[[${inputs.goal}]]\n- 溝通管道與我的顧慮：[[${inputs.channel_tone}]]\n\n請輸出：\n① 【一句話定調】— 開口前先幫我把心態擺正的一句提醒（例：目標是解決事情、不是討回公道），30 字以內\n② 【溝通訊息主文】— 一則可直接傳 LINE 或抄進聯絡簿的訊息，100-150 字，依『先客觀描述事件 → 說出我的擔心／需要 → 提出具體請求』的順序，客氣、清楚、有合作感\n③ 【聯絡簿精簡版】— 濃縮成 30-50 字、適合手寫進聯絡簿的版本\n④ 【老師三種反應的接話】— 若老師『積極配合／委婉推託／略帶防衛』，各給我一句怎麼回，維持合作又守住訴求\n⑤ 【地雷清單】— 這次溝通絕對不能說的 3 句話，每句附一句為什麼會壞事\n\n【規則】\n1. 格式：訊息要口語、可直接複製貼上 LINE 或抄進聯絡簿，繁體中文、台灣家長日常用語；不要用生硬公文腔。\n2. 嚴禁使用：「你們老師怎麼可以這樣」「我要去投訴／找校長」「我小孩在家明明很乖」「你們要負責」——這種恐龍家長式的指控會讓老師瞬間豎起防衛，把事情推向對立。\n3. 運用非暴力溝通（觀察→感受需要→請求）與『親師是教育合夥人』的立場：先描述事實不預設老師惡意，把訴求包裝成『我們一起幫孩子』，而不是『你做錯了』。\n4. 語氣：像一個尊重老師專業、同時堅定守護孩子的家長，理性、誠懇、不卑不亢，不情緒勒索也不低聲下氣。\n5. 驗證測試：想像老師讀完的反應應該是「這位家長很好溝通，我願意幫忙」，而不是「又一個難搞的家長」；若讀起來像興師問罪就重寫。並在結尾附一句提醒：「若涉及霸凌、身體受傷或安全疑慮，除了與導師溝通，也請同時循學校正式管道（輔導室、學務處）處理，必要時保留紀錄。」`
  },

  // ━━━ 🔵 中階咒文 | 職場求生 | Free ━━━
  {
    id: "freelance_quote_forge",
    tab: "職場求生",
    isPro: false,
    tier: "adept",
    school: "contract" as SchoolType,
    subSchool: "attack" as SchoolType,
    outputFormat: "報價訊息 + 三方案分級 + 守價話術",
    icon: <Coins className="w-8 h-8 text-amber-500" />,
    color: "amber",
    title: "報價煉金：接案開價術",
    desc: "接設計、家教、攝影、剪輯、文案、手作、代購——客戶一句『這樣大概多少錢？』你就開始冒冷汗：開太高怕直接被已讀，開太低又做到懷疑人生、還被凹著一改再改？更慘的是熟人來找，一句『幫個忙、算便宜點啦』你就守不住價，範圍越滾越大，最後時薪比打工還低。問題不是你不會做，是沒人教你怎麼把價講出口、怎麼把範圍框好。這咒語當你接案十年的報價教練，先幫你抓出這類服務的合理行情、你這案該落在哪，把它切成基本／標準／進階三個方案用價格錨定引導客戶往上加，生出一段可以直接傳給客戶、含服務範圍、修改次數、交期與付款方式的專業報價訊息，再備好被殺價、被凹、熟人情商時不傷和氣又守得住價的回應，讓你開口有底氣、接案不做白工。",
    tags: ["接案報價", "自由工作者", "守價話術"],
    fields: [
      { id: "service", label: "你提供什麼服務", placeholder: "例：接 logo 設計 / 國中數學家教 / 婚禮攝影 / 短影音剪輯 / 幫小店寫 IG 文案" },
      { id: "scope", label: "這次案子的內容與規模", placeholder: "例：一張主視覺＋3 張延伸 / 每週 2 小時共 8 週 / 拍 4 小時交 80 張修圖 / 3 支 60 秒影片" },
      { id: "cost_time", label: "你的成本、時間與行情概念", placeholder: "例：大概要花 20 小時、想時薪 500 起 / 同行大概收 8000-12000 / 完全不確定行情" },
      { id: "client_worry", label: "客戶狀況與你最怕的事", placeholder: "例：熟人不好意思開口 / 對方一直說預算有限 / 怕被無限改稿 / 怕開太高被已讀不回" },
    ],
    tweak: {
      id: "strat",
      label: "報價策略",
      options: [
        "行情定錨型：先算出合理價格區間並以中高價定錨，讓最終成交落在你理想的數字",
        "方案分級型：主打切成基本／標準／進階三方案，用選項引導客戶往上加價、也給彼此台階",
        "守價圓融型：專門處理被殺價、被凹改稿、熟人情商，教你不傷和氣地守住價格與工作範圍",
      ]
    },
    modules: [
      { type: 'role' as ModuleType, label: '角色設定', preview: '接案十年的報價教練' },
      { type: 'decision' as ModuleType, label: '判斷邏輯', preview: '依成本與行情抓價位' },
      { type: 'output' as ModuleType, label: '輸出格式', preview: '報價訊息＋分級＋守價' },
      { type: 'behavior' as ModuleType, label: '行為規則', preview: '明訂範圍與改稿次數' },
      { type: 'safety' as ModuleType, label: '安全邊界', preview: '不賤賣不做白工' },
    ],
